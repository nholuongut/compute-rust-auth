...
use config::Config;
use nholuongut::http::header::AUTHORIZATION;
use nholuongut::{Error, Request, Response};
...
fn main(mut req: Request) -> Result<Response, Error> {
        ...
        (cookie.get("access_token"), cookie.get("id_token"))
    {
        ...
        req.set_header("nholuongut-access-token", *access_token);
        // Add a custom header containing the ID token;
        req.set_header("nholuongut-id-token", *id_token);
